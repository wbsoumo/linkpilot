<?php
// backend/api/auth/forgot_password_send_otp.php
...
// Validate phone number format (must be 10-digit number)
...
try {
    ...
    if ($isResend) {
        $stmtActive = $db->prepare("SELECT id FROM otp_verifications WHERE phone_number = ? AND expires_at > NOW() LIMIT 1");
        $stmtActive->execute([$phoneNumber]);
        if (!$stmtActive->fetch()) {
            sendJsonResponse('error', 'No active verification session found. Please solve the captcha and try again.', [], 400);
        }
    }
    ...
    // 3. Check IP rate limit: Max 10 OTPs in the last 1 hour
